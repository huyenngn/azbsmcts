...
\textbf{How can AlphaZero-inspired methods be adapted to imperfect information games
...
This section reviews the background concepts and prior work that are directly relevant to this thesis. The goal is not to provide a comprehensive survey, but to introduce the key ideas needed to understand the problem formulation, methods, and experimental results.
...
Because the true board state is hidden, a player must reason about multiple possible game states that are consistent with the observations received so far. Two different hidden states may appear identical to the player while differing in strategically important ways. This makes Phantom Go significantly more challenging than standard Go and unsuitable for algorithms that assume full observability.
...
During search, actions are evaluated across multiple sampled states, and their outcomes are aggregated to guide decision-making. This allows the agent to account for uncertainty explicitly rather than treating it as noise. BS-MCTS has been shown to outperform IS-MCTS in Phantom games, including Phantom Go, particularly as game complexity increases \cite{Wang2015bsmcts}.
...
Several approaches have been proposed to extend learning-based methods to imperfect information games, often by combining search with belief modeling or alternative solution concepts. However, applying AlphaZero-style methods to games like Phantom Go remains challenging due to the large state space and the difficulty of representing uncertainty effectively \cite{Brown2020rebel}.
...
This section defines the problem addressed in this thesis in precise but practical terms. Rather than aiming for full game-theoretic generality, the formulation is tailored to Phantom Go as implemented in the OpenSpiel framework and to the agents studied in the subsequent sections.
...
The key difference from standard Go is that players do not observe the opponent's stones on the board. At any point during the game, the true board configuration is hidden. Instead, players receive partial feedback signals related to moves and captures. These signals restrict the set of possible board states but never fully determine it.
...
This section describes the Belief-State Monte Carlo Tree Search (BS-MCTS) baseline used in this thesis. BS-MCTS provides a principled way to apply search-based decision making in imperfect information games by explicitly reasoning over uncertainty about the hidden game state. The presentation here focuses on the practical variant implemented for Phantom Go and highlights differences from the original formulation.
...
\begin{itemize}
  ...
  \item The \textbf{value head} estimates the expected game outcome from the perspective of the player to move.
\end{itemize}
...
This section describes the experimental setup used to evaluate the proposed AlphaZero-inspired belief-based agent and to compare it against search-based baselines. The goal of the evaluation is to assess how learning-based search performs under imperfect information and how it compares to non-learning belief-state methods under controlled computational budgets.

\subsection{Experimental Setup}

All experiments are conducted on Phantom Go using the OpenSpiel implementation. Games are played on a standard $9 \times 9$ board with the default komi value of $7.5$.
...
Only the AlphaZero-inspired belief agent requires training. Training is performed through self-play using belief-state MCTS guided by the current neural network.

Unless otherwise specified, the following configuration is used:
\begin{itemize}
  \item Board size: $9 \times 9$
  \item Number of simulations per move ($T$): 8
  \item Number of belief samples per move ($S$): 4
  \item Training epochs per iteration: 5
  \item Batch size: 64
  \item Learning rate: $10^{-3}$
\end{itemize}

To study the effect of training budget, multiple models are trained with increasing numbers of self-play games. In particular, training budgets of 100, 300, 600, and 1000 self-play games are considered. All other parameters are held constant across these runs.
