...
Board games are popular domains for artificial intelligence (AI) research because
they provide controlled environments with clear rules and objectives.
In recent years, learning-based approaches have achieved great success in games with perfect
information, where all players can observe the full game state at any given time.
One example is AlphaZero, which combines deep neural networks with
Monte Carlo Tree Search (MCTS) and has reached superhuman performance in games
like Chess, Shogi and Go \cite{Silver2017alphazero}.

Games with imperfect information, however, present a different set of challenges.
In these games, players do not have access to the full game state and must make decisions based on partial observations.
This uncertainty requires agents to reason over multiple possible hidden states of the game,
which can lead to fundamentally different strategic considerations compared to perfect information games
and remains a challenge for AI methods to handle effectively.


This thesis aims to explore how learning-based search methods, inspired by AlphaZero,
can be adapted to handle the challenges of imperfect information games.
The focus is on Phantom Go, a variant of Go where players cannot see the opponent's stones.
...
to a single guessed reality \cite{Wang2015bsmcts}.

At the same time, AlphaZero-style methods rely on assumptions that do not
...
To investigate this question, this thesis focuses on integrating belief-state reasoning into the AlphaZero framework.
...
The key difference from standard Go is that players do not observe the opponent's stones on the board. At any point, the true board configuration is hidden.
