...
The search and training parameters used in this thesis were chosen to balance computational feasibility with experimental clarity. Rather than tuning parameters for maximum performance, all agents are evaluated under the same fixed computational budget to ensure a fair comparison. The number of simulations and belief samples per move was chosen to provide a minimal but non-trivial search budget that allows meaningful comparison between agents while remaining computationally feasible. As a result, the reported parameters should be interpreted as controlled defaults rather than optimized settings.
