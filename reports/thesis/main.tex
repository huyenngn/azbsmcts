...
This thesis builds on prior work in belief-state search and learning-based game playing.
...
\label{sec:implementation_details_game}
...
Players can also stall by repeating the same observational moves that do not change the board state.
...
The cutoff point is chosen to balance the constraints of computation time while still providing a buffer big enough for observational exploration and any prolonged strategic maneuvering.

A game length discount factor is applied to the value of terminal states, encouraging agents to win more quickly and lose more slowly.
This avoids setting off the cutoff and leads the game to a more natural conclusion.
...
After preliminary testing, a board size of $5 \times 5$ is chosen to balance computational feasibility with strategic complexity, as larger boards significantly increased the state space which led to constant particle collapse and prohibitive training times under the available resources.

In accordance to the modifications described in Section~\ref{sec:implementation_details_game},
a maximum game length of 125 moves is enforced to prevent infinite games and a discount factor of 0.999 is applied to terminal values to encourage timely conclusions.
