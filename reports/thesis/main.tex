...
Only the AlphaZero-inspired belief agent requires training. Training is performed through self-play using belief-state MCTS guided by the current neural network. The training configuration below was selected after minimal exploratory tuning (i.e., small trial runs to verify that learning proceeds and that search remains stable under the chosen budget), rather than a systematic hyperparameter optimization procedure.
...
To study the effect of training budget, multiple models are trained with increasing numbers of self-play games. In particular, training budgets of 100, 300, 600, and 1000 self-play games are considered, while all other parameters are held constant across runs.

Overall, the search and training parameters used in this thesis were chosen to balance computational feasibility with experimental clarity. Rather than tuning parameters for maximum performance, all agents are evaluated under the same fixed computational budget per move to ensure a fair comparison. The number of simulations and belief samples per move was chosen to provide a minimal but non-trivial search budget that enables meaningful comparisons between agents while remaining computationally feasible. Consequently, the reported parameters should be interpreted as controlled defaults rather than optimized settings.
